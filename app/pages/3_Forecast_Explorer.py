...
def load_product_options_with_names(schema_pl: str, schema_gd: str):
    ...
    try:
        ...
        FROM {schema_pl}.{FORECAST_TABLE} f
...
target_type_selection = st.radio(
    "🔍 Loại dự báo (Target Type)",
    options=["💰 Doanh thu (Revenue)", "📦 Số lượng (Quantity)"],
    horizontal=True,
    help="Chọn loại dự báo: Revenue (doanh thu) hoặc Quantity (số lượng đơn vị bán)"
)

# Map selection to target type and table name
if "Revenue" in target_type_selection:
    TARGET_TYPE = "revenue"
    FORECAST_TABLE = "demand_forecast_revenue"
    TARGET_LABEL = "Doanh thu (BRL)"
    TARGET_COL = "revenue"
    UNIT = "BRL"
else:
    TARGET_TYPE = "quantity"
    FORECAST_TABLE = "demand_forecast_quantity"
    TARGET_LABEL = "Số lượng (đơn vị)"
    TARGET_COL = "quantity"
    UNIT = "đơn vị"

st.markdown(f"**📊 Đang hiển thị dự báo: {TARGET_TYPE.upper()}**")
st.markdown("---")
...
with tab_forecast:
    ...
    # Try to check both tables (revenue and quantity)
    has_forecast_table = check_table_exists(FORECAST_TABLE, SCHEMA_PL)
    # Fallback to demand_forecast for backward compatibility
    if not has_forecast_table:
        has_forecast_table = check_table_exists("demand_forecast", SCHEMA_PL)
        if has_forecast_table:
            FORECAST_TABLE = "demand_forecast"  # Use old table name
    
    if not has_forecast_table:
        st.info("""
        📊 **Bảng `platinum.{FORECAST_TABLE}` chưa được tạo.**
        
        Để tạo dữ liệu forecast, vui lòng:
        1. Mở **Dagster UI** (http://localhost:3001)
        2. Vào **Jobs** → `forecast_job`
        3. Click **Launch Run** với target_type={TARGET_TYPE}
        4. Đợi pipeline hoàn thành (4 bước: features → train → predict → monitor)
        5. Refresh trang này để xem kết quả
        
        Kết quả sẽ được lưu vào `platinum.{FORECAST_TABLE}`
        
        💡 **Lưu ý**: Bạn có thể train cả Revenue và Quantity models riêng biệt.
        """)
        ...
        try:
            ...
            regions_sql = f"SELECT DISTINCT region_id FROM {SCHEMA_PL}.{FORECAST_TABLE} WHERE region_id IS NOT NULL ORDER BY region_id LIMIT 50"
            regions_df = run_query(regions_sql, SCHEMA_PL)
            regions = regions_df["region_id"].tolist() if not regions_df.empty else []
            
            date_range_sql = f"SELECT MIN(forecast_date) AS min_date, MAX(forecast_date) AS max_date FROM {SCHEMA_PL}.{FORECAST_TABLE}"
            ...
            try:
                ...
                check_cols_sql = f"SELECT * FROM {SCHEMA_PL}.{FORECAST_TABLE} LIMIT 1"
        ...
        if not forecast_df.empty:
            ...
            with scenario_col1:
                ...
                st.metric("🔴 Kịch bản thận trọng", f"{sum_lo:,.0f} {UNIT}", delta=f"{sum_lo - forecast_df['yhat'].sum():,.0f} {UNIT}", delta_color="inverse", help="Tổng yhat_lo - worst case scenario")
            with scenario_col2:
                sum_base = scenario_df["yhat"].sum()
                st.metric("🟡 Kịch bản cơ sở", f"{sum_base:,.0f} {UNIT}", help="Tổng yhat - base scenario")
            with scenario_col3:
                sum_hi = scenario_df["yhat_hi"].sum()
                st.metric("🟢 Kịch bản lạc quan", f"{sum_hi:,.0f} {UNIT}", delta=f"{sum_hi - forecast_df['yhat'].sum():,.0f} {UNIT}", help="Tổng yhat_hi - best case scenario")
            ...
            if "forecast_date" in forecast_df.columns:
                ...
                fig.update_layout(
                    title=f"{chart_title} - {TARGET_LABEL}",
                    xaxis_title="Ngày",
            ...
            if "horizon" in forecast_df.columns:
                ...
                fig_h = px.bar(
                    ...
                    x="horizon", 
                    y="yhat", 
                    title=f"{horizon_title} - {TARGET_LABEL}", 
                    labels={"horizon": "Horizon (ngày)", "yhat": f"Forecast TB ({UNIT})"},
                    hover_data={"yhat": ":,.0f"}
                )
                fig_h.update_layout(
                    height=350, 
...
# ==============
with tab_monitoring:
    st.markdown("### 📊 Forecast Monitoring & Quality")
    
    # Filter by target type
    monitoring_target = target_type_selection.replace("💰 Doanh thu (Revenue)", "revenue").replace("📦 Số lượng (Quantity)", "quantity")
    
    has_monitoring_table = check_table_exists("forecast_monitoring", SCHEMA_PL)
    has_detail_table = check_table_exists("forecast_backtest_detail", SCHEMA_PL)
    
    if not has_monitoring_table:
        st.info("📊 **Bảng `platinum.forecast_monitoring` chưa được tạo.** Bảng này được tạo tự động khi chạy backtest trong `forecast_job`.")
    else:
        # Load latest metrics (filtered by target_type if column exists)
        metrics_sql = f"""
        SELECT 
            horizon, model_type, metric, value, n_samples, created_at, run_id, target_type
        FROM {SCHEMA_PL}.forecast_monitoring
        WHERE target_type = '{TARGET_TYPE}' OR target_type IS NULL
        ORDER BY created_at DESC, horizon, model_type, metric
        LIMIT 1000
        """
        metrics_df = run_query(metrics_sql, SCHEMA_PL)
        
        if not metrics_df.empty:
            st.success(f"✅ Đã tải {len(metrics_df):,} dòng metrics cho target: {TARGET_TYPE}")
            
            # Filter to latest run (most recent created_at)
            if "created_at" in metrics_df.columns:
                latest_date = pd.to_datetime(metrics_df["created_at"].max())
                metrics_df_latest = metrics_df[pd.to_datetime(metrics_df["created_at"]) == latest_date].copy()
            else:
                metrics_df_latest = metrics_df.copy()
            
            # Pivot metrics for easier analysis
            if "metric" in metrics_df_latest.columns and "value" in metrics_df_latest.columns:
                df_wide = metrics_df_latest.pivot_table(
                ...
                df_lgb = df_wide[df_wide["model_type"] == "lightgbm"].copy()
                
                if len(df_lgb) > 0 and "smape" in df_lgb.columns:
                    overall_smape = df_lgb["smape"].mean()
                    h1_smape = df_lgb[df_lgb["horizon"] == 1]["smape"].iloc[0] if len(df_lgb[df_lgb["horizon"] == 1]) > 0 else None
                    h7_smape = df_lgb[df_lgb["horizon"] == 7]["smape"].iloc[0] if len(df_lgb[df_lgb["horizon"] == 7]) > 0 else None
                    h14_smape = df_lgb[df_lgb["horizon"] == 14]["smape"].iloc[0] if len(df_lgb[df_lgb["horizon"] == 14]) > 0 else None
                    h28_smape = df_lgb[df_lgb["horizon"] == 28]["smape"].iloc[0] if len(df_lgb[df_lgb["horizon"] == 28]) > 0 else None
                    
                    # CI Coverage
                    ci_coverage_df = metrics_df_latest[
                        (metrics_df_latest["metric"] == "ci_coverage") & 
                        (metrics_df_latest["model_type"] == "lightgbm")
                    ]
                    overall_coverage = ci_coverage_df["value"].mean() if len(ci_coverage_df) > 0 else None
                    
                    kpi_col1, kpi_col2, kpi_col3, kpi_col4, kpi_col5 = st.columns(5)
                    with kpi_col1:
                        st.metric("sMAPE (Overall)", f"{overall_smape:.1f}%", help="Trung bình sMAPE trên tất cả horizons")
                    with kpi_col2:
                        if h1_smape is not None:
                            st.metric("sMAPE H1", f"{h1_smape:.1f}%", help="sMAPE cho horizon 1 ngày")
                    with kpi_col3:
                        if h7_smape is not None:
                            st.metric("sMAPE H7", f"{h7_smape:.1f}%", help="sMAPE cho horizon 7 ngày")
                    with kpi_col4:
                        if h28_smape is not None:
                            st.metric("sMAPE H28", f"{h28_smape:.1f}%", help="sMAPE cho horizon 28 ngày")
                    with kpi_col5:
                        if overall_coverage is not None:
                            st.metric("CI Coverage", f"{overall_coverage*100:.1f}%", help="Tỷ lệ điểm thật nằm trong CI")
                    
                    # Threshold warnings
                    st.markdown("---")
                    if overall_smape > 35:
                        st.error(f"⚠️ **Chất lượng dự báo thấp**: sMAPE overall = {overall_smape:.1f}% > 35%. Cần xem lại model / dữ liệu.")
                    elif overall_smape > 30:
                        st.warning(f"⚠️ **Chất lượng dự báo trung bình**: sMAPE overall = {overall_smape:.1f}%. Cân nhắc cải thiện model.")
                    else:
                        st.success(f"✅ **Chất lượng dự báo tốt**: sMAPE overall = {overall_smape:.1f}%")
                    
                    if overall_coverage is not None:
                        if overall_coverage < 0.75:
                            st.warning(f"⚠️ **CI quá hẹp**: Coverage = {overall_coverage*100:.1f}% < 75%. CI không đáng tin.")
                        elif overall_coverage > 0.98:
                            st.warning(f"⚠️ **CI quá rộng**: Coverage = {overall_coverage*100:.1f}% > 98%. CI quá 'an toàn'.")
                        else:
                            st.success(f"✅ **CI hợp lý**: Coverage = {overall_coverage*100:.1f}%")
                    
                    # Chart: Horizon Decay (LightGBM vs Baseline)
                    st.markdown("---")
                    st.markdown("#### 📈 Horizon Decay: LightGBM vs Baseline")
                    df_baseline = df_wide[df_wide["model_type"] == "naive_last"].copy()
                    
                    if len(df_lgb) > 0 and len(df_baseline) > 0 and "smape" in df_lgb.columns and "smape" in df_baseline.columns:
                        fig_decay = go.Figure()
                        ...
                        fig_decay.update_layout(
                            title="sMAPE theo Horizon: LightGBM vs Naive Baseline",
                            xaxis_title="Horizon (ngày)",
                            yaxis_title="sMAPE (%)",
                            height=400,
                            template="plotly_dark",
                            plot_bgcolor="rgba(0,0,0,0)",
                        ...
                        if len(df_lgb) > 0 and len(df_baseline) > 0:
                            improvements = []
                            for h in df_lgb["horizon"].unique():
                                lgb_h = df_lgb[df_lgb["horizon"] == h]["smape"].iloc[0] if len(df_lgb[df_lgb["horizon"] == h]) > 0 else None
                                naive_h = df_baseline[df_baseline["horizon"] == h]["smape"].iloc[0] if len(df_baseline[df_baseline["horizon"] == h]) > 0 else None
                                if lgb_h is not None and naive_h is not None:
                                    improvement = naive_h - lgb_h
                                    improvements.append(f"H{h}: {improvement:+.1f} điểm ({improvement/naive_h*100:+.1f}%)")
                            
                            if improvements:
                                st.caption(f"💡 **Cải thiện so với baseline**: {', '.join(improvements)}")
                    
                    # Top/Bottom Series Analysis
                    if has_detail_table:
                        st.markdown("---")
                        st.markdown("#### 📊 Top/Bottom Series theo sMAPE")
                        
                        detail_sql = f"""
                        SELECT 
                            product_id, region_id, horizon, y, yhat
                        FROM {SCHEMA_PL}.forecast_backtest_detail
                        WHERE target_type = '{TARGET_TYPE}' OR target_type IS NULL
                        LIMIT 10000
                        """
                        detail_df = run_query(detail_sql, SCHEMA_PL)
                        
                        if not detail_df.empty and "y" in detail_df.columns and "yhat" in detail_df.columns:
                            ...
                            series_metrics = (
                                detail_df.groupby(["product_id", "region_id"])
                                .apply(smape_series)
                                .reset_index(name="smape")
                            )
                            
                            # Add volume (total y for priority)
                            series_vol = detail_df.groupby(["product_id", "region_id"])["y"].sum().reset_index(name="total_volume")
                            series_metrics = series_metrics.merge(series_vol, on=["product_id", "region_id"], how="left")
                            ...
                            try:
                                ...
                                SELECT DISTINCT
                                    dp.product_id,
                                    COALESCE(pc.product_category_name_english, dp.product_category_name, dp.product_id) AS product_name
                                FROM {SCHEMA_GD}.dim_product dp
                                LEFT JOIN {SCHEMA_GD}.dim_product_category pc
                                  ON dp.product_category_name = pc.product_category_name
                                """
                                product_df = run_query(product_sql, SCHEMA_GD)
                                if not product_df.empty:
                                    series_metrics = series_metrics.merge(product_df, on="product_id", how="left")
                                    series_metrics["product_name"] = series_metrics["product_name"].fillna(series_metrics["product_id"])
                            except:
                                pass  # Fallback to product_id if join fails
                            
                            series_tabs = st.tabs(["🔴 Top 20 Series Tệ nhất", "🟢 Top 20 Series Tốt nhất"])
                            
                            with series_tabs[0]:
                                top_bad = series_metrics.nlargest(20, "smape")
                                top_bad = top_bad.sort_values("smape", ascending=False)
                                # Reorder columns for better display
                                display_cols = ["product_name", "product_id", "region_id", "smape", "total_volume"]
                                display_cols = [c for c in display_cols if c in top_bad.columns]
                                st.dataframe(top_bad[display_cols], use_container_width=True, height=400)
                                st.caption("💡 Các series có sMAPE cao nhất → cần xem lại model hoặc dữ liệu. Ưu tiên các series có volume cao.")
                            
                            with series_tabs[1]:
                                top_good = series_metrics.nsmallest(20, "smape")
                                top_good = top_good.sort_values("smape", ascending=True)
                                display_cols = ["product_name", "product_id", "region_id", "smape", "total_volume"]
                                display_cols = [c for c in display_cols if c in top_good.columns]
                                st.dataframe(top_good[display_cols], use_container_width=True, height=400)
                                st.caption("💡 Các series có sMAPE thấp nhất → model dự báo tốt. Có thể dùng làm reference cho các series khác.")
                    ...
                    st.download_button("⬇️ Tải CSV", csv, f"forecast_metrics_{TARGET_TYPE}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv", use_container_width=True)
                else:
                    st.info("📊 Chưa có metrics sMAPE trong dữ liệu.")
            else:
                st.info("📊 Không có dữ liệu metrics.")
        else:
            st.info("📊 Không có dữ liệu monitoring cho target type này.")

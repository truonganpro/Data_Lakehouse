    ...
    **🎯 Hành động**: Bật Moving Average 3M trên chart; theo dõi "rising categories" (thay đổi Pareto theo tháng).
    ...
    try:
        ...
        if not df_growth.empty:
            ...
            import numpy as np
            df_growth_sorted = df_growth.sort_values("year_month").copy()
            df_growth_sorted["gmv_ma3"] = df_growth_sorted["gmv"].rolling(window=3, min_periods=1).mean()
            
            fig_area = px.area(df_growth_sorted, x="year_month", y="gmv",
                              title="GMV theo thời gian (Area Chart)",
                              labels={"gmv": "GMV (BRL)", "year_month": "Tháng"})
            # Add Moving Average line
            fig_area.add_trace(go.Scatter(
                x=df_growth_sorted["year_month"],
                y=df_growth_sorted["gmv_ma3"],
                mode="lines+markers",
                name="MA(3)",
                line=dict(color="rgba(255, 193, 7, 0.9)", width=2.5, dash="dot"),
                marker=dict(size=5)
            ))
...
with tabs[6]:
    ...
    * Tóm tắt **quy mô & chất lượng khách hàng**: số khách, repeat rate, GMV per customer.
    * Phân tích **phân bố số đơn / khách** (1 lần vs repeat).
    * Xem **Pareto khách hàng**: top khách đóng góp bao nhiêu % GMV.
    * Phân tích **khách hàng theo bang (state)** để hỗ trợ mở rộng thị trường & SLA.
    """)
    ...
    try:
        ...
        WITH base AS (
            SELECT 
                fo.customer_id,
                d.year_month,
                SUM(COALESCE(fo.sum_price, 0) + COALESCE(fo.sum_freight, 0)) AS gmv,
                COUNT(DISTINCT fo.order_id) AS orders
            FROM {SCHEMA_GD}.fact_order fo
            JOIN {SCHEMA_GD}.dim_date d 
                ON fo.full_date = d.full_date
            WHERE fo.full_date >= DATE '{start_date}' 
              AND fo.full_date < DATE '{end_next}'
              AND NOT fo.is_canceled
            GROUP BY fo.customer_id, d.year_month
        )
        SELECT 
            customer_id,
            SUM(orders)               AS total_orders,
        ...
        """
        df_cust = run_sql(sql_cust, SCHEMA_GD)

        if df_cust.empty:
            st.info("Không có dữ liệu khách hàng cho khoảng thời gian đã chọn.")
        else:
            # ===========================
            # 2. KPI Summary cho Customer
            # ===========================
            total_customers = int(df_cust["customer_id"].nunique())
            repeat_customers = int((df_cust["total_orders"] >= 2).sum())
            repeat_rate = (repeat_customers / total_customers * 100) if total_customers else 0.0
            avg_orders_per_cust = float(df_cust["total_orders"].mean()) if total_customers else 0.0
            avg_gmv_per_cust = float(df_cust["total_gmv"].mean()) if total_customers else 0.0

            total_gmv_cust = float(df_cust["total_gmv"].sum())
            # Pareto: đóng góp của Top-N khách (dùng chung slider topn ở sidebar)
            df_cust_sorted = df_cust.sort_values("total_gmv", ascending=False)
            top_n = min(topn, len(df_cust_sorted))
            top_gmv = float(df_cust_sorted.head(top_n)["total_gmv"].sum())
            top_share = (top_gmv / total_gmv_cust * 100) if total_gmv_cust > 0 else 0.0

            c1, c2, c3, c4 = st.columns(4)
            with c1:
                kpi_tile("Total Customers", f"{total_customers:,}", "Số khách unique trong kỳ")
            with c2:
                kpi_tile(
                    "Repeat Customers",
                    f"{repeat_customers:,}",
                    f"Repeat rate ≈ {repeat_rate:.1f}%",
                    "positive" if repeat_rate >= 10 else ""
                )
            with c3:
                kpi_tile("Avg Orders / Customer", f"{avg_orders_per_cust:.2f}", "Số đơn TB mỗi khách")
            with c4:
                kpi_tile("Avg GMV / Customer (BRL)", f"{avg_gmv_per_cust:,.0f}", f"Top {top_n} khách ≈ {top_share:.1f}% GMV")
            ...
            * Có **{total_customers:,} khách** trong kỳ; trong đó **{repeat_customers:,} khách** có từ 2 đơn trở lên (**~{repeat_rate:.1f}%**).
            * Mỗi khách trung bình có **{avg_orders_per_cust:.2f} đơn**, GMV TB khoảng **{avg_gmv_per_cust:,.0f} BRL**.
            * **Top {top_n} khách** đóng góp khoảng **{top_share:.1f}%** tổng GMV → mức độ tập trung doanh thu theo khách hàng.
            """)

            # =========================================
            # 3. Phân bố số đơn / khách (Frequency dist)
            # =========================================
            def bucket_orders(n):
                if n == 1:
                    return "1"
            ...
            df_cust["order_bucket"] = df_cust["total_orders"].apply(bucket_orders)
            df_bucket = (
                df_cust.groupby("order_bucket")
                .agg(
            ...
            df_bucket = df_bucket.sort_values("customers", ascending=True)  # sort để bar đẹp

            c_left, c_right = st.columns([1.5, 1.5])
            with c_left:
                st.markdown("#### Phân bố số đơn / khách")
                fig_freq = px.bar(
                    df_bucket,
                    x="order_bucket",
                    y="customers",
                    labels={"order_bucket": "Số đơn / khách", "customers": "Số khách"},
                    title="Số khách theo nhóm số đơn",
                    text="customers",
                )
            ...
            with c_right:
                st.markdown("#### Đóng góp GMV theo nhóm số đơn")
                # thêm % share để đọc rõ hơn
                df_bucket["gmv_share"] = (
                    df_bucket["gmv"] / df_bucket["gmv"].sum() * 100
                    if df_bucket["gmv"].sum() > 0
                    else 0
                )
                fig_bucket_gmv = go.Figure()
                fig_bucket_gmv.add_bar(
                    x=df_bucket["order_bucket"],
                    y=df_bucket["gmv"],
                    name="GMV",
                )
                fig_bucket_gmv.add_scatter(
                    x=df_bucket["order_bucket"],
                    y=df_bucket["gmv_share"],
                    name="GMV Share (%)",
                    yaxis="y2",
                    mode="lines+markers",
                )
                fig_bucket_gmv.update_layout(
                    title="GMV & % share theo nhóm số đơn",
                    yaxis=dict(title="GMV (BRL)"),
                    yaxis2=dict(
                        title="% GMV",
                        overlaying="y",
                ...
                st.plotly_chart(fig_bucket_gmv, use_container_width=True)

            st.markdown("---")

            # ==================================
            # 4. Pareto khách hàng (Top-N GMV)
            # ==================================
            st.markdown(f"#### Pareto: Top {top_n} khách theo GMV")

            df_pareto = df_cust_sorted.copy()
            df_pareto["cum_share"] = df_pareto["total_gmv"].cumsum() / df_pareto["total_gmv"].sum() * 100

            df_pareto_top = df_pareto.head(top_n).copy()
            df_pareto_top["customer_rank"] = range(1, len(df_pareto_top) + 1)

            fig_cust_pareto = go.Figure()
            fig_cust_pareto.add_bar(
                x=df_pareto_top["customer_rank"],
                y=df_pareto_top["total_gmv"],
                name="GMV",
            )
            fig_cust_pareto.add_scatter(
                x=df_pareto_top["customer_rank"],
                y=df_pareto_top["cum_share"],
                name="Cumulative %",
                yaxis="y2",
                mode="lines+markers",
                line=dict(width=3, dash="dot"),
            )
            fig_cust_pareto.update_layout(
                title="Pareto khách hàng (xếp hạng theo GMV)",
                xaxis=dict(title="Thứ hạng khách (1 = cao nhất)"),
                yaxis=dict(title="GMV (BRL)"),
                yaxis2=dict(
                    title="Cumulative %",
                    overlaying="y",
            ...
            st.plotly_chart(fig_cust_pareto, use_container_width=True)

            st.caption(
                f"💡 **Diễn giải**: Đường đỏ cho thấy mức độ tập trung GMV. "
                f"Ví dụ, nếu tại rank 50 cumulative ≈ 60% thì 50 khách đầu tiên mang lại 60% doanh thu."
            )

            st.markdown("---")

            # ======================================
            # 5. Phân tích khách hàng theo Bang/State
            # ======================================
            st.markdown("#### Phân bố khách hàng & GMV theo Bang (State)")
            ...
            try:
                ...
                HAVING COUNT(DISTINCT fo.customer_id) > 0
                ORDER BY gmv DESC
                LIMIT 20
                """
                df_state = run_sql(sql_state, SCHEMA_GD)
                ...
                if not df_state.empty:
                    ...
                    with c_s1:
                        fig_state_cust = px.bar(
                            ...
                            x="state",
                            y="customers",
                            title="Số khách theo Bang",
                            labels={"state": "State", "customers": "Customers"},
                        )
                        fig_state_cust.update_layout(
                            height=380,
                            template="plotly_dark",
                            plot_bgcolor="rgba(0,0,0,0)",
                            paper_bgcolor="rgba(0,0,0,0)",
                    ...
                    with c_s2:
                        fig_state_gmv = px.bar(
                            ...
                            y="gmv",
                            title="GMV theo Bang",
                            labels={"state": "State", "gmv": "GMV (BRL)"},
                        )
                        fig_state_gmv.update_layout(
                            height=380,
                            template="plotly_dark",
                            plot_bgcolor="rgba(0,0,0,0)",
                            paper_bgcolor="rgba(0,0,0,0)",
                        )
                        st.plotly_chart(fig_state_gmv, use_container_width=True)

                    st.caption(
                        "💡 **Gợi ý**: Các bang có **nhiều khách nhưng GMV thấp** → cơ hội tăng AOV. "
                        "Bang có GMV cao nhưng ít khách → tập trung mở rộng tệp khách."
                    )
                else:
                    st.info("Không có dữ liệu state cho khoảng thời gian này.")
            except Exception as e:
                st.warning(f"Không thể tải phân tích theo bang: {e}")

            st.markdown("---")

            # =========================
            # 6. Bảng chi tiết (sample)
            # =========================
            st.markdown("#### Bảng chi tiết khách hàng (Top theo GMV)")
            df_detail = df_cust_sorted.head(min(200, len(df_cust_sorted))).copy()
            df_detail = df_detail.rename(
                columns={
                    "customer_id": "customer_id",
                    "total_orders": "total_orders",
                    "total_gmv": "total_gmv",
                    "first_ym": "first_year_month",
                    "last_ym": "last_year_month",
                }
            )
            st.dataframe(
                df_detail.style.format(
                    {"total_gmv": "{:,.0f}", "total_orders": "{:,.0f}"}
                ),
...
with tabs[7]:
    ...
    try:
        # Get payment totals from dm_payment_mix
        sql_p = f"""
        SELECT 
            payment_type,
        ...
        # Get installments from fact_payment (for credit_card only)
        sql_inst_avg = f"""
        ...
        LIMIT 1
        """
        df_inst_avg = run_sql(sql_inst_avg, SCHEMA_GD)
        ...
        if not df_p.empty:
            ...
            avg_installments = float(df_inst_avg['avg_installments'].iloc[0]) if not df_inst_avg.empty and not pd.isna(df_inst_avg['avg_installments'].iloc[0]) else 0
        else:
            credit_card_pct, avg_installments = 0, 0
    except Exception as e:
        st.error(f"Lỗi khi lấy payment stats: {e}")
        credit_card_pct, avg_installments = 0, 0
...
with tabs[8]:
    ...
    **📊 Dữ liệu dự báo đã được chuyển sang Forecast Explorer** - vui lòng sử dụng trang **Forecast Explorer** trong menu bên trái.
    ...
    if st.button("📊 Mở Forecast Explorer", use_container_width=True, type="primary"):
        st.switch_page("pages/3_Forecast_Explorer.py")
